...
| `external` | Standalone brand or client/family work with its own identity. Lives on its own TLD (or no TLD). | donkeyco, imaginalvision.com, jpheinnet, poppasblog, jphein-wordpress-server, jphein-wordpress-site, forageforall, jewelrycycle, shawnahein.com, solar, sdp, sdp+disability-appeal, starcharts, roblox, claudedoublehours (extension on store), VoxSherpa-TTS, storyvox, clawwatch. |
